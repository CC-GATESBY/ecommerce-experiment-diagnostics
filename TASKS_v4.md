...
**本轮更新：S0、T0.1、T0.2、T0.3 的既有结果保留；T0.4 的 REES46 10 月文件获取、登记与 100,000 条工程样本已通过验收。Criteo 子项未开始，整个 T0.4 和 G0 仍未完成。** 勾选只表示已检查的对应产物，不表示后续代码或分析已运行。本人独立解释项保留未勾选，未替用户声明通过。
...
- [x] REES46 原始文件可定位且校验值可复查；表头与数据记录分开；重复登记和同内容换名被识别，同名异内容拒绝；失败下载不能登记完整。完整行数、完整时间范围保持 `not_measured`。
...
本轮源文件下载载荷为 1,741,928,540 bytes，解压 CSV 为 5,668,612,855 bytes；不是仅下载 100,000 行。样本时间范围为 2019-10-01 00:00:00–04:28:27 UTC，解析失败 0 条。Kaggle 目录版本 8 已核实，但原发布方外链对象没有独立版本号，未与 Kaggle 文件逐字节比较。整月 ETL、事实 Parquet、业务指标和 T1.1 均未运行。G0 继续未勾选。

### T1.1 解析事实表并保留质量标记（P0）

- [ ] 已实现并记录实际输入范围。
- [ ] 输入记录数=事实层记录数（或有效事实+显式隔离记录数）；无静默丢弃；金额精度溢出有错误记录；只有通过质量门禁的日期进入后续分析。跨月追加不覆盖已有日期。
- [ ] 证据已保存：`etl/01_events.py`；`sql/ddl/events.sql`；`warehouse/fact_events/`；`reports/data_quality.csv`
