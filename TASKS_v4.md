...
**本轮更新（2026-09-17）：T2.2描述性报告、8图及授权范围内核验已完成，整项保留in_progress。唯一缺口为品类整月去重users/buyers：日维度人数不可跨日相加，本轮仅授权小时事实聚合，故未扩大查询；暂存用户日/月去重not_measured，待用户确认范围。T2.1历史验收不变；T2.3、T4.1–T4.4仍not_started，Criteo/T0.4与全项目G0/G1保持原状态，本人解释未代验。**
...
- [ ] 品类整月去重users/buyers：现有433行日维度不能还原月去重，未获额外查询授权。CSV已区分user_days/buyer_user_days与月去重not_measured，不将人数相加冒充通过。
- [ ] 能独立解释：给业务同事一分钟，最重要的发现是什么？它影响哪些人或商品？

状态：`in_progress`　run_id：`behavior-month-01 / hourly-month-01`　实际完成日期：`2026-09-17（已授权报告部分）；整项未完成`　阻塞：`品类月去重人数需要范围决定，不能从日汇总推造`
