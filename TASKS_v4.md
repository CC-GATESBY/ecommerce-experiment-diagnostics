...
**当前更新（2026-09-24）：E1 done（有限基线训练＋valid开发比较；非最终模型确认，本人解释未代验）。四个分类器串行完成，原train内部显式早停；30%同容量的响应/S/T相对简单规则G差分别+3.8462/+3.9802/+2.4634，完整成对区间及名单重叠保留。仅保留响应模型为新增开发候选，不新增test评价或调参；E2/E3、CUPED、T7.3/G2不代勾。**

**历史更新（2026-09-23）：POWER-01 done（固定队列、未调整购买率功效模拟；本人解释未代验）。9档×300全部可计算；零效应双侧12/300，S1参考283/300，完整区间与失败分母保留。T5.7仅raw先行子项完成，整体in_progress；T5.6/CUPED和raw/CUPED比较未执行，T7.3/G2仍待本人解释。**

**历史更新（2026-09-23）：COST-01 done（人工成本与需求条件分析；非真实利润或税务核定，本人解释未代验）。27个人工篮子、双参照及有限需求情景通过核验；不读真实数据、不调整旧业务结论。T7.3/G2仍待本人解释，不新增模型或政策计算平台。**

**历史更新（2026-09-22）：TARGET-02 done（冻结简单规则的test留出评价完成，本人解释未代验）。30%主比较差+2.6064/万候选记录，成对区间[1.7130,3.4390]；规则不重学，两种定向名单仍相同。正向结果支持目标业务试验准备，不批准部署；E1、完整E2、G2、T7.3及本人解释项不提前通过。**

**历史更新（2026-09-22）：TARGET-01与T3.3有限f0/f1描述分层done（train/valid开发评价，本人解释未代验）。30%同人数下简单定向相对随机有正向线索；基础响应与增量排序实际名单相同，尚无额外复杂度依据。test仅经历必要物理字节/身份对齐，未用于本轮特征/标签分析；E1/E2/G2和本人解释项不提前通过。**

**历史更新（2026-09-21）：PRICE-01同商品价格关联＋条件贡献毛利门槛done（业务增强，本人解释未代验）。9个候选按支持度固定3个；A无同期价格对照，B日期补充稀疏且方向敏感，C只有5条成功。只提出取证/有条件试验准备，不批准调价；T7状态和历史结果保持，Criteo定向任务未开始。**

**历史更新（2026-09-21）：T7.1求职版导航/隔离人工复现、T7.2简历证据映射/阶段审查完成；本人解释未代验。T7.3仅准备练习，保持in_progress，本人待验，G2未通过。原结果与历史报告不改，不追加数据或方法。**

**历史更新（2026-09-19）：computers六日商品构成定向核查done（工程与业务补充，本人解释未代验）。记录数量及两期购买商品组合变化的证据强于同商品金额下调；优先收窄到B六日记录与规格对账，以A单日集中为基准核查对照。不进入价格实验/T4.5/T5。**
...
**历史更新（2026-09-18）：T4业务增强（10–11月持续性及品类差异）done，工程及报告交付，本人解释未代验。旧十月结果不改；优先核对11月15–17日总体购买事件覆盖/时间，再对computers偏弱日期定向查SKU构成。不进入其他月份、T4.5或T5。**
...
| [x] | T3.3 | P1 | 有限的描述性异质性分析（train/valid，本人解释未代验） | T3.2 |
...
| [x] | T7.1 | P0 | 生成可追溯README与运行手册 | T2.3、T3.2、T3.4、T4.4、T5.4、T5.5 |
| [x] | T7.2 | P0 | 简历证据映射与阶段完成审查 | T7.1 |
| [ ] | T7.3 | P0 | 理解验收与现场修改练习 | T7.2 |
| [x] | E1 | P2 | 有限基线训练＋valid开发比较；非最终模型确认，本人解释未代验 | T3.1、T3.3、T7.1 |
...
- [x] 仅原Criteo train/valid、预指定f0/f1，用train边界形成3个互斥档，conversion唯一结果。
- [x] 报告每档两臂n/成功/率/差及描述区间；低支持回退、稀疏保护通过；区间非同时推断，不以显著性重排或声称正式异质性检验。
- [x] 证据复用本次实际产物：`uplift/targeting.py`、`reports/targeting/stratum_summary.csv`、`reports/targeting/frozen_rules.json`、`reports/targeting_coverage_review.md`及`docs/targeting_validation.md`；不另写重复模块/报告。
- [ ] 能独立解释：描述性分层、CATE建模、正式异质性检验有什么区别？

状态：`done`（有限描述分层，本人解释未代验）　run_id：`targeting-valid-01`　实际完成日期：`2026-09-22`。非复杂模型、非最终test；原test切片未参与选择。
...
- [x] POWER-01 raw先行子项：唯一cohort-oct-01，固定Y0；9档、每档300轮，事前冻结独立随机流及同轮共享U/分组；复用未调整购买率Wald-z。无金额、CUPED、数据扩量或模型。
- [x] raw结果完整登记：2,700次有效，失败0，300次分配SRM0；零效应双侧4.00%〔2.30%,6.86%〕，S1参考94.33%〔91.11%,96.43%〕。理论80%位置约0.457pp，经验点在0.40→0.46pp跨越，不强报精确模拟MDE。
- [x] raw证据：`config/power_raw.json`、`docs/power_raw_contract.md`、`abtest/power.py`、`scripts/run_power_raw.py`、`tests/test_power_raw.py`、`reports/power_raw_runs.csv`、`reports/power_raw_summary.csv`、`reports/power_raw_report.md`、`docs/power_raw_validation.md`。旧A/A及p=.0712单次注入不变。
- [ ] 原T5.7完整范围（含raw/CUPED比较）已实现并记录实际输入范围。
...
状态：`in_progress`（POWER-01 raw先行子项done；本人解释未代验）　run_id：`power-raw-oct-01`　子项完成日期：`2026-09-23`　剩余：`T5.6未做；原T5.7的raw/CUPED共同分组比较未完成，不将本轮子项代替整体验收`
...
- [x] README按三个业务判断组织，三类证据和技术/规模边界分开；历史README保留于8be3b17提交，原阶段报告不改。
- [x] 临时隔离venv与仅代码/SQL副本实际运行12条synthetic事件演示；固定名单/分母字面预期和稀疏推断边界通过，无.local依赖。未重跑真实数据或原Spark环境。
- [x] 证据：`README.md`、`docs/runbook.md`、`docs/result_registry.csv`、`scripts/demo_synthetic.py`、`reports/project_release_checklist.md`；原`reports/period_summary.md`作为历史阶段证据保留。
- [ ] 能独立解释：没有原始大文件，评审者怎样验证你的逻辑？哪个run_id支持核心结论？

状态：`done`（文档与最小隔离复现；本人解释未代验）　run_id：`t7-synthetic-isolated-01`　实际完成日期：`2026-09-21`　阻塞：`无工程差异；仅人工路径复现，非全量重新验收`

### T7.2 简历证据映射与阶段完成审查（P0）

- [x] 三条中文简历草稿围绕经营判断、未上线试验设计和结果解释，逐项绑定结果、run、原commit及限制。
- [x] 数字/链接/规模/证据类型核对；公开样本观察、随机实验来源与人工模拟不冒充本人业务收益。原岗位维度文档仍为T0.1历史快照，本轮已证实能力与缺口见新审查。
- [x] 证据：`reports/resume_evidence.md`、`reports/project_release_checklist.md`、`docs/result_registry.csv`。G0/G1工程证据逐项列出，G2本人验收、P1/P2未完成项保留。
- [x] 2026-09-25模型专题材料收尾：三条主项目保留经营/价格判断、购物车试验条件、模型与简单规则同容量评价；valid与旧test追加主差分开登记完整精度及限制。POWER-01仅未调整功效、CUPED对照未做；只读已提交汇总，不新增分析或改历史结果。
- [ ] 能独立解释：项目在哪些要求上能证明能力，哪些需要实习或协作经历补充？

状态：`done`（证据映射与阶段审查；本人解释未代验）　run_id：`t7-evidence-review-01`　实际完成日期：`2026-09-21`　阻塞：`无文档差异；不是线上系统或全部个人能力认证`

### T7.3 理解验收与现场修改练习（P0）

- [x] 练习材料已准备：90秒参考稿、三个业务案例提纲、八个追问、SQL/执行计划、配置预测与边界测试练习；参考思路和本人作答区分开。
- [x] 模型专题补充Q12–Q15：logloss与增量排序、接近与等价、旧test正区间的证据性质、G与部署条件；仅问题及参考思路，本人作答/修改/通过均pending，原Q1–Q11及动手练习未代做。
- [ ] 本人不照稿解释粒度/处理边界/下一步；独立手写SQL、解释执行计划、预测配置变化并修改测试；知道CATE需要设计或额外识别假设。
- [x] 材料：`reports/interview_notes.md`；`reports/learning_log.md`尚未形成本人实际练习记录，不虚构创建或填入完成情况。
- [ ] 能独立解释：哪些部分由工具辅助？哪些判断是你作出的？遇到负面结果如何解释？

状态：`in_progress`（练习材料已准备，本人待验；不能标done）　run_id：`t7-evidence-review-01`（仅材料）　实际完成日期：`本人验收未完成`　剩余：`本人独立作答、SQL/计划、配置预测及测试修复；G2尚未通过`

### E1 训练响应/S/T三个有限基线（P2）

- [x] 原train 8,387,273条内固定fit 7,549,516 / early_stop 837,757；valid 2,797,762条。十二DOUBLE特征；仅S训练额外包含原treatment，T按原臂分别训练；无负采样、valid早停或test业务读取。
- [x] 四分类器固定参数串行训练、保存和加载；全部valid条件预测逐值复现。常数logloss整体/分臂对照通过，模型结果不以必须胜出为验收条件。
- [x] 30%五策各839,328条；旧简单规则名单/计数/G不变。三个模型对简单规则的联合bootstrap开发比较、重叠及负/稀疏/失败状态保留；原TARGET、POWER、COST及raw/membership不改。
- [x] 证据：`config/uplift_baselines.json`、`uplift/models.py`、`scripts/run_uplift_baselines.py`、`tests/test_uplift_models.py`、`reports/uplift_model_card.md`、三份`reports/uplift_*.csv`、`docs/uplift_baselines_protocol.md`、`docs/uplift_baselines_validation.md`。模型、ID、缓存和预测只留本地。
- [ ] 能独立解释：响应预测与增量预测区别是什么？为什么样本少的对照组会增加估计不稳定？

状态：`done`（有限基线训练＋valid开发比较；非最终模型确认，本人解释未代验）　run_id：`uplift-valid-01`　实际完成日期：`2026-09-24`　后续限制：`保留响应模型为唯一新增开发候选，原简单规则继续作基准；原test已用于TARGET-02，本轮不重用；E2/E3及T7.3/G2未验，不自动继续调参`
...
### PRICE-01 同商品价格关联＋毛利门槛（独立业务增强）

- [x] 复用唯一十月funnel-month-01，只读授权路径列；formal数量与成功标记对上原T2.1。
- [x] 冻结不依赖结果的精确价格对与支持条件；9候选中按支持度固定3个，用户覆盖跨商品独立去重。
- [x] 保留非购买路径，比较原始关联/共同日期补充，按用户整簇1,000次重抽并披露稀疏与不可比状态。
- [x] 9个纯假设毛利格、12个实际价格对条件情景分开；不是ROI、价格因果或真实上线收益。
- [x] 人工边界、紧凑汇总独立算术、固定seed复验和输入不可变通过；旧事实/漏斗/结论及T7不改。
- [ ] 能独立解释：B的日期补充为何不能证明降价有效？A点估计超过门槛为何仍不能调价？同商品为何不等于同规格？

状态：`done`（可行性、关联与条件门槛完成，本人解释未代验）。run_id：`price-margin-03`。证据：`reports/price_margin_review.md`、`reports/price_analysis/`、`docs/price_margin_validation.md`。A/C无共同日期支持、B/C成功稀疏是结果限制，未换样本或放宽标准。01/02失败收据本地保留；不将完成专题解释为实证因果成立。下一轮Criteo相同覆盖下定向与随机增量比较须单独执行；本轮未读取其明细/test或选择特征。

### TARGET-01 同覆盖人数下的定向规则比较（独立业务增强）

- [x] 原raw与membership逐逻辑ordinal对齐，保留原身份/划分；只生成train/valid六列缓存，test特征/标签不保存或聚合。
- [x] train分位/支持/回退/分数先冻结；三规则只用固定分数和同一hash排序，valid不重学。
- [x] 10/20/30/50/100%同容量精确人数、嵌套；30%为唯一主比较，不择容量/seed/特征追结果。
- [x] 完整联合选中标记bootstrap保留策略重叠，直接评价G差；100%结果及差区间守恒。
- [x] 人工隔离与分布/矩测试、真实计数/冻结证据/独立算术通过；三份小CSV及冻结文件脱敏发布。
- [ ] 能独立解释：为何RESPONSE和INCREMENTAL名单相同？同人数与同成本有何区别？为何当前不能部署或称test已验收？

状态：`done`（valid开发评价，本人解释未代验），run_id=`targeting-valid-01`。主差+3.0217/万候选记录、成对区间[2.1421,3.8292]；RESPONSE与INCREMENTAL各容量名单相同，无需为本版增量排序增加复杂度。证据：`reports/targeting_coverage_review.md`、`reports/targeting/`、`docs/targeting_validation.md`。保留原T7、PRICE-01及其他历史结果；E1复杂建模、E2最终test、真实预算/ROI和本人理解未验。下一步是否做test，须审阅并固定方案后单独授权，本轮停止。

### TARGET-02 冻结简单规则的test留出评价（独立业务增强）

- [x] test结果读取前保存协议；核对原冻结规则及TARGET-01完成收据，不调用训练、改变边界或名次。
- [x] 原CSV与membership逐逻辑ordinal对齐，只提取原test五列；人数、两臂QC、身份/序列SHA精确一致。
- [x] 预定五容量同人数且嵌套，30%为唯一主比较；两种定向完整排序一致，100%及同名单策略差精确0。
- [x] 原联合类型bootstrap、1,000次、seed/区间/稀疏状态保持；保留所有预定容量结果，无调参或重评估。
- [x] 4项新人工测试、5项直接相关旧回归及97项真实检查通过；valid原结果直接引用、不重算、不合并，原raw/membership/规则/旧报告不改。
- [ ] 能独立解释：留出正向比较为何仍不是实际新增客户或ROI？为什么相同定向名单不算两套成功证据？目标业务迁移缺什么？

状态：`done`（冻结简单规则的test留出评价完成，本人解释未代验）。run_id=`targeting-test-01`，日期`2026-09-22`；工程完成与结果正负分开。主差+2.6064/万候选记录、成对区间[1.7130,3.4390]，valid正向线索获得本公开样本的留出支持。证据：`reports/targeting_holdout_review.md`、`reports/targeting_test/`、`docs/targeting_holdout_protocol.md`、`docs/targeting_holdout_validation.md`。不冒充E1、完整E2或真实线上实验；T7.3/G2和本人解释仍待验。下一步只建议取得目标业务处理前特征、干预实验及实际成本评价迁移，本轮不自动继续。

### COST-01 固定进口成本与篮子选择（人工经营情景附录）

- [x] 冻结27个人工篮子和主案例，Decimal计算六方案；提价商品成本不变，增购商品/履约成本同步提高。
- [x] 并列无新增费用与ABSORB参照，输出单篮子差、概率门槛和五档行动需求敏感性；非正贡献不输出误导比例。
- [x] 官方背景有限核对、日期与访问限制记录；H不替代真实税则归类，VAT净额假设及遗漏费用明确。
- [x] 12项人工测试、独立Fraction完整核对、跨进程字节复现与已有目录保护通过；原数据、规则、结果和依赖锁不改。
