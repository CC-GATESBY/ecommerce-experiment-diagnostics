...
| [ ] | T0.3 | P0 | 仓库、版本锁定与最小 Spark 验证 | T0.2 |
...
- [ ] 已实现并记录实际输入范围。
- [ ] 小型 Parquet 写入、读回及聚合均成功；执行器和 Python 包 Spark 版本一致；配置缺失时报错；raw、私钥、token、临时数据不入 Git。smoke 只能证明基本环境，不能代表全量性能。
- [ ] 证据已保存：`config/local.yaml`；`requirements.lock.txt`；`scripts/smoke_spark.py`；`tests/test_config.py`；`.gitignore`
...
- [ ] 已实现并记录实际输入范围。
- [ ] 原始文件可定位且校验值可复查；表头行与数据行分开；同一文件重复导入被识别；相同内容换名也不会重复累计。下载失败、缺月份或条款不明写明阻塞，不伪造数据。
- [ ] 证据已保存：`data/MANIFEST.md`；`data/manifest.json`；`ingest/ingest.py`；`docs/data_sources.md`
