...
**本轮更新：保留既有环境、下载验收及两份头部工程样本；REES46 完整源覆盖与固定用户候选输入已核验，源 42,448,764 条，候选 151,121 用户 / 2,114,081 条事件，均观察到 10 月 31 天。Criteo 来源验收后移未取消；整个 T0.4 与 G0 仍未完成，T1.1、T1.5 未开始。** 勾选只表示已检查的对应产物，不表示后续代码或分析已运行。本人独立解释项保留未勾选，未替用户声明通过。
...
- [ ] T1.1（小样本子任务）：读CSV、标记质量、写Parquet、读回；用小型日指标做独立核对。
...
| [ ] | T1.1 | P0 | 解析事实表并保留质量标记 | T0.4 |
| [ ] | T1.2 | P0 | 重复候选与会话分布核验 | T1.1 |
| [ ] | T1.3 | P0 | 建设统一口径的指标层 | T1.1、T1.2 |
| [ ] | T1.4 | P0 | 独立交叉核验与重复运行测试 | T1.3 |
...
- [ ] 输入记录数=事实层记录数（或有效事实+显式隔离记录数）；无静默丢弃；金额精度溢出有错误记录；只有通过质量门禁的日期进入后续分析。跨月追加不覆盖已有日期。
...
状态：`in_progress`　旧工程 run_id：`engineering-20260916-03`、`engineering-20260916-04`　工程及整月子项验收日期：`2026-09-16`　整项完成日期：`未完成`　剩余：`日期质量放行规则被后续分析消费的验收、跨月追加不覆盖验收；本人解释仍待确认`
...
此前工程核验不替代 T1.4 DuckDB 正式验收；本轮整月输入已获单独授权如下，正式指标仍未放行。T1.1 整体、T1.2–T1.5、T0.4、G0/G1 与本人解释项保持未完成；不自动进入下一任务。

本轮整月候选解析子任务（旧报告和输出保留）：
...
- [ ] 未满足的整项验收：后续分析消费日期质量放行规则、跨月追加不覆盖的实测验收。没有本轮授权的 11 月真实输入；可下一轮用人工跨月文件先验证。本人解释项不代勾。

本轮没有实现 T1.2–T1.5 或正式指标/实验，没有把候选提升为最终分析范围；T0.4/Criteo、总 G0/G1 保持原状态。唯一下一步建议：单独收口 T1.1 日期质量放行与人工跨月追加验收，然后再考虑后续任务。

### T1.2 重复候选与会话分布核验（P0）

- [ ] 已实现并记录实际输入范围。
- [ ] 缺失 session 的记录不会被误标为“共同的超大会话”；有关联前后行数/金额核对；去重/高频过滤只进入独立敏感性分析，报告相对主口径的差异，不污染正式表。
- [ ] 证据已保存：`sql/quality/duplicate_candidates.sql`；`sql/quality/session_profile.sql`；`reports/quality_policy.md`
...
- [ ] 已实现并记录实际输入范围。
- [ ] 主键唯一；无分母时输出 null+状态而非无穷；满足条件的日期有 V=U×(B/U)×(V/B) 恒等式核验；每个维度金额加总等于同范围总金额；同一报告注明金额缺失范围。
- [ ] 证据已保存：`docs/metric_contract.md`；`sql/metrics/`；`warehouse/agg_user_daily/`；`warehouse/agg_daily_metrics/`；`warehouse/agg_daily_dim/`
...
- [ ] 已实现并记录实际输入范围。
- [ ] 行数、去重用户数、购买事件数一致；Decimal 金额按同一精度精确核对；派生浮点比率用预设容差。原始过滤范围不同必须解释并修复，不能通过扩大容差掩盖。重跑结果不翻倍，历史分区不消失。
- [ ] 证据已保存：`tests/crosscheck_duckdb.py`；`tests/test_idempotency.py`；`reports/crosscheck.csv`
