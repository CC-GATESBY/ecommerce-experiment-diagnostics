...
[业务报告](reports/aa_validation.md)用T5.1固定队列进行300次50:50哈希分组，结果不变。购买率显著21次（7.00%，Wilson 4.62%–10.46%），每用户金额17次（5.67%，3.57%–8.89%）；无SRM或失败。随机差值中间95%范围分别约−0.3414至+0.3498个百分点、−6.0735至+7.2061金额单位。固定例子0001恰好两项正向显著，仍没有对应真实处理，不能转成发券或预算依据。

[600行逐次结果](reports/aa_runs.csv) · [两套校准汇总](reports/aa_summary.csv) · [统计契约](docs/statistical_contract.md) · [验收和复跑](docs/t54_validation.md)。T5.2–T5.4已完成工程验收，本人解释未代验；ratio方法只用人工数据与成对bootstrap核验。T5.5、CUPED和功效曲线未执行，未证明线上平台可靠或任何真实策略有效。
...
每用户一行，身份与数值用稳定键连接并仅存本地；金额3,499,563.53，每入组用户41.5798，完整观测而非利润。仅复用已核验用户日表，未启动Spark或重建指标；[验收及复跑](docs/t51_validation.md) · [脱敏汇总](reports/cohort_baseline.csv)。T5.1已通过固定队列与分母验收，本人解释未代验；该阶段未执行T5.2–T5.5，后续A/A状态见上文。
...
优先核对product_B六日同规格购买记录，以product_A的单日集中记录作历史对照；其余高金额对象不少只有2条历史购买，仍是样本线索。3份脱敏汇总、1张图和[验收](docs/computers_product_mix_validation.md)已完成；完整商品结果/别名映射只留本地。旧scope、指标和报告不改，未读源/候选CSV或启动Spark；不自动执行价格实验、T4.5或T5。

## 11月购买记录的限定核查

[购买时间核查](reports/purchase_timing_review.md)已将15日零购买及16–17日峰值定位到公开源：同哈希选中记录、现有候选及日指标精确对齐，窗口purchase无原九字段完全相同重复候选。它排除了本轮范围内的本地提取/汇总差异，未确认真实业务或发布前记录原因。

完整11月日均较十月+23.64%；暂不纳入15–17日的事后分支为−2.58%，不替代主结果。22日相对历史均值的方向随15日历史资格改变，但检测中位数两支均下降。继续使用完整日志描述，对增长、恢复和受影响历史比较附条件；优先取得同范围交易侧时间对账。旧双月报告和数据保持原样，详情及复核命令见[验收](docs/purchase_timing_validation.md)。本轮停止，不自动进入computers核查。
