...
from etl.event_config import (ALLOWED, CONTRACT, DERIVED, FLAGS, HEADER,
                              PROVENANCE, load_events_config, sha256)
...
def transform(raw, config, run_id):
    ...
    blank = lambda name: F.col(name).rlike(r'^[ \t\r\n\x0b\x0c]*$')
    pattern = "yyyy-MM-dd HH:mm:ss 'UTC'"
    exact = F.col('event_time').rlike(r'^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2} UTC$') & ~F.col('event_time').startswith('0000-')
    ...
    for name in ('user_id', 'product_id', 'category_id'):
        df = df.withColumn(name + '_missing', blank(name)).withColumn(name + '_invalid', ~blank(name) & ~F.col(name).rlike(r'^[0-9]+$'))
    ...
    lexical = F.col('price').rlike(r'^[+-]?[0-9]+(?:\.[0-9]+)?$')
    nonfinite = F.lower(F.col('price')).rlike(r'^[+-]?(?:s?nan|inf|infinity)$')
    integral = F.regexp_replace(F.regexp_extract('price', r'^[+-]?([0-9]+)', 1), '^0+', '')
    fractional = F.regexp_extract('price', r'\.([0-9]+)$', 1)
...
def summary(df):
    ...
    # Exactly one aggregate row is returned; fact rows are never collected.
    result = df.agg(*aggregates).first().asDict()
...
def main():
    ...
    try:
        spark = SparkSession.builder.appName('T1.1-engineering-only').getOrCreate()
        ...
        receipt.update(status='passed', summary=actual_summary, schema=schema().jsonValue())
